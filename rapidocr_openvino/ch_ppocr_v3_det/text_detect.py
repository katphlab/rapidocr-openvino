...
class TextDetector:
    ...
    def __call__(
        self, img: np.ndarray
    ) -> Tuple[Optional[list[list[int]]], Optional[np.ndarray], float]:
        ...
        polygons, dt_boxes, dt_boxes_scores = self.postprocess_op(preds, ori_img_shape)
        dt_boxes = self.filter_tag_det_res(dt_boxes, ori_img_shape)
        elapse = time.perf_counter() - start_time
        return polygons, dt_boxes, elapse
