...
class RapidOCR:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Optional[List[List[Union[Any, str]]]],
        Optional[List[List[Union[Any, str]]]],
        Optional[List[float]],
    ]:
        ...
        polygons, dt_boxes, cls_res, rec_res = None, None, None, None
        det_elapse, cls_elapse, rec_elapse = 0.0, 0.0, 0.0

        if use_det:
            img, padding_h = self.maybe_add_letterbox(img)
            polygons, dt_boxes, det_elapse = self.auto_text_det(img)
            if dt_boxes is None:
        ...
        ocr_res = self.get_final_res(
            polygons, dt_boxes, cls_res, rec_res, det_elapse, cls_elapse, rec_elapse
        )
        return ocr_res
    ...
    def auto_text_det(
        self, img: np.ndarray
    ) -> Tuple[Optional[list[list[int]]], Optional[List[np.ndarray]], float]:
        polygons, dt_boxes, det_elapse = self.text_det(img)
        if dt_boxes is None or len(dt_boxes) < 1:
            return None, None, 0.0

        dt_boxes = self.sorted_boxes(dt_boxes)
        return polygons, dt_boxes, det_elapse
    ...
    def get_final_res(
        self,
        polygons: Optional[List[List[int]]],
        ...
        if dt_boxes is None and rec_res is None and cls_res is not None:
            return None, cls_res, [cls_elapse]

        if dt_boxes is not None and rec_res is None and cls_res is not None:
            return (
                polygons,
                [[box.tolist() for box in dt_boxes], cls_res],
                [det_elapse, cls_elapse],
            )
        ...
        if dt_boxes is None and rec_res is not None:
            return None, [[res[0], res[1]] for res in rec_res], [rec_elapse]

        if dt_boxes is not None and rec_res is None:
            return polygons, [box.tolist() for box in dt_boxes], [det_elapse]
...
def main():
    ...
    _, result, elapse_list = ocr_engine(
        args.img_path,
        use_det=use_det,
        use_cls=use_cls,
        use_rec=use_rec,
