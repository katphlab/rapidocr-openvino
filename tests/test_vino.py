...
from rapidocr_openvino import LoadImageError, RapidOCR  # noqa: E402
...
def test_only_det():
    _, result, _ = engine(img_path, use_det=True, use_cls=False, use_rec=False)
...
def test_only_cls():
    ...
    _, result, _ = engine(img_path, use_det=False, use_cls=True, use_rec=False)
    assert len(result) == 1
    assert result[0][0] == "0"
...
def test_only_rec():
    ...
    _, result, _ = engine(img_path, use_det=False, use_cls=False, use_rec=True)
    assert len(result) == 1
    assert result[0][0] == "韩国小馆"


def test_det_rec():
    _, result, _ = engine(img_path, use_det=True, use_cls=False, use_rec=True)
...
def test_cls_rec():
    img_path = tests_dir / "text_cls.jpg"
    _, result, _ = engine(img_path, use_det=False, use_cls=True, use_rec=True)
...
def test_input_str():
    _, result, _ = engine(str(img_path))
    assert result[0][1] == "正品促销"
    assert len(result) == 18
...
def test_det():
    ...
    img = cv2.imread(str(img_path))
    _, dt_boxes, elapse = text_det(img)
